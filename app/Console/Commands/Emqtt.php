...
class Emqtt extends Command
{
    ...
    public function generAction()
    {
        $cmd = [
            ...
            'GETDEVATTR' => '获取设置',
            'SETDEVATTR' => '修改设置',
            'DEVICE_MODEL' => '设备设备模型信息获取',
            'STOP_PLAY' => '设备停止播放某个气味 ',
            'REMOVE_TASK' => '移除设备任务',
        ];
        $prefix = [
            'SCI_REQ_',
        ...
        $i = 0;
        foreach ($cmd as $k => $v) {
            echo "{$prefix[0]}{$k} = 1000" . ($i + 1) . ";\t// {$cmd[$k]} request;" . PHP_EOL;
//             echo "{$prefix[1]}{$k} = " . ($i * 2 + 2) . ";\t// {$cmd[$k]} response;" . PHP_EOL;
            $i ++;
        }
        
        echo PHP_EOL;
        echo '// Response CMD'.PHP_EOL;
        $i = 0;
        foreach ($cmd as $k => $v) {
//             echo "{$prefix[0]}{$k} = " . ($i * 2 + 1) . ";\t// {$cmd[$k]} request;" . PHP_EOL;
            echo "{$prefix[1]}{$k} = 2000" . ($i + 1) . ";\t// {$cmd[$k]} response;" . PHP_EOL;
            $i ++;
        }
    }
    ...
    public function gSupportCmdAction()
    {
        $cmd = [
            'SLEEP' => '主机休眠',
            'WAKEUP' => '唤醒主机',
        ...
        foreach ($cmd as $k => $v) {
            echo "Simple.SrCmdId.{$prefix[0]}{$k}," . PHP_EOL;
        }
        foreach ($cmd as $k => $v) {
            echo "Simple.SrCmdId.{$prefix[1]}{$k}," . PHP_EOL;
        }
    }
    ...
    {
        $mqtt = new MQTT("tcp://192.168.5.61:1883/",'PHP-client-1');
//         $mqtt = new MQTT("tcp://120.26.109.169:1883/", 'PHP-client-1');
    ...
    public function testAction()
    {
        $this->template(function ($mqtt) {
            $topics['/9c32626fc323/resp'] = 1;
            $topics['/xfwq22ixZyX7UBdIsqir'] = 1;
            //             $topics['/TCeOp0gzzrWhAMoOa3Mm/resp'] = 1;
            $topics['/9c32626fc323'] = 1;
...
//         $InitalizationKey = "sdfsdfdsfsdfs"; // Set the inital key
    ...
    public function tAction(){
        ...
        $str = file_get_contents(public_path('a.data'));
        dump(strlen($str));
    }
